...
/**
 * Represents a Waf rule, ensuring that no runs happen after the rule is destroyed and that the rule
 * is not destroyed during runs.
 */
public class WafHandle implements Closeable {
  ...
  WafHandle(String uniqueName, WafConfig config, Map<String, Object> definition)
    ...
    if (!definition.containsKey("version")) {
      throw new UnclassifiedWafException("Invalid definition. Expected key 'version' to exist");
    }

    if (!definition.containsKey("events") && !definition.containsKey("rules")) {
  ...
  public Waf.ResultWithData runRules(
      Map<String, Object> parameters, Waf.Limits limits, WafMetrics metrics)
      throws AbstractWafException {
    this.readLock.lock();
    try {
      checkIfOnline();
      ...
      try {
        ...
        if (newLimits.generalBudgetInUs == 0L) {
          LOGGER.debug(
              "Budget exhausted after serialization; " + "not running rule of context {}", this);
          throw new TimeoutWafException();
        }
        res = Waf.runRules(this.handle, lease.getFirstPWArgsByteBuffer(), newLimits, metrics);
      } finally {
        lease.close();
        if (metrics != null) {
      ...
      throw new UnclassifiedWafException(
          "Error calling Waf's runRule for rule in context " + this + ": " + rte.getMessage(), rte);
    } finally {
      this.readLock.unlock();
    }
  }
  ...
  public WafHandle update(String uniqueId, Map<String, Object> specification)
      throws AbstractWafException {
