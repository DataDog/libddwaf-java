...
jobject builder, jstring path, jobject configuration, jobject diagnostics) {
    ...
    if (memcmp(&ddwaf_diagnostics, &(ddwaf_object) {0},
            ...
            if (JNI(ExceptionCheck)) {
                java_wrap_exc("Error deleting internal reference for RuleSetInfo");
                goto error;
            }
        }
...
JNIEXPORT jobject JNICALL Java_com_datadog_ddwaf_WafBuilder_buildInstance(JNIEnv *env, jclass clazz, jobject builder_java, jobject old_handle_java) {
    ...
    if(old_handle_java != NULL) {
        JAVA_LOG(DDWAF_LOG_DEBUG, "Destroying old handle");
        ddwaf_handle old_handle = get_pwaf_handle_checked(env, old_handle_java);
        if (old_handle == NULL) {
            JAVA_LOG(DDWAF_LOG_DEBUG, "Failed to get old handle");
        } else {
            JAVA_LOG(DDWAF_LOG_DEBUG, "Successfully got old handle to destroy");
...
JNIEXPORT void JNICALL Java_com_datadog_ddwaf_WafHandle_destroyWafHandle(JNIEnv *env, jclass clazz, jlong waf_handle) {
    UNUSED(clazz);
...
static bool _cache_methods(JNIEnv *env)
{
    ...
    if (!java_meth_init_checked(env, &_pwaf_handle_init,
                                "com/datadog/ddwaf/WafHandle", "<init>",
...
static int64_t _get_pw_run_timeout_checked(JNIEnv *env)
{
    ...
    env_key = java_utf8_to_jstring_checked(
                env, "DD_APPSEC_WAF_TIMEOUT", strlen("DD_APPSEC_WAF_TIMEOUT"));
    if (!env_key) {
        goto end;
    }
    ...
    if (*end != '\0') {
        JAVA_LOG(DDWAF_LOG_WARN, "Invalid value of system property "
                           "DD_APPSEC_WAF_TIMEOUT: '%s'", val_cstr);
        goto end;
    }

    JAVA_LOG(DDWAF_LOG_INFO, "Using value %lld us for DD_APPSEC_WAF_TIMEOUT", val);
