...
static inline bool _has_attribute(const ddwaf_object *res);
                                      ...
                                      const ddwaf_object *ddwaf_result)
{
    ...
    jboolean has_events = JNI_TRUE; // Default to true when NULL/missing
    ...
    if (events_obj != NULL && events_obj->type == DDWAF_OBJ_ARRAY) {
        if (ddwaf_object_size(events_obj) > 0) {
            ...
            // If events array is empty, set to true (default)
            has_events = JNI_TRUE;
        }
    }
        ...
        if (attributes_obj) {
            ...
            if (attr_json) {
                ...
                if (attr_json_str) {
                    ...
                    JAVA_LOG(DDWAF_LOG_DEBUG,
                             "[JNI] attributes_obj as JSON: %s", attr_json_str);
                    free(attr_json_str);
                }
                json_seg_free(attr_json);
            } else {
                JAVA_LOG(DDWAF_LOG_DEBUG,
                         "[JNI] attributes_obj JSON conversion failed");
            }
            JAVA_LOG(DDWAF_LOG_DEBUG,
                     "[JNI] attributes_obj type: %d, size: %llu",
                     (int) attributes_obj->type,
                     (unsigned long long) ddwaf_object_size(attributes_obj));
        } else {
            JAVA_LOG(DDWAF_LOG_DEBUG, "[JNI] attributes_obj is NULL");
        }
    }
    // --- END LOGGING ---
    if (attributes_obj != NULL && attributes_obj->type == DDWAF_OBJ_MAP &&
        ...
        if (!attributes) {
            JAVA_LOG(DDWAF_LOG_ERROR,
                     "[JNI] Failed encoding inferred attributes");
